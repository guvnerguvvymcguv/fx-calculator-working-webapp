...
serve(async (req) => {
  ...
  // Get all active MONTHLY export schedules
  ...
  const currentDay = ukTime.dayOfMonth;
  const currentHour = ukTime.hour;

  console.log('=== Monthly Export Check ===');
  console.log('Current UTC time:', now.toISOString());
  console.log('UK timezone:', ukTime.isBST ? 'BST (UTC+1)' : 'GMT (UTC+0)');
  console.log('UK Day of Month:', currentDay, 'UK Hour:', currentHour);
  console.log('Force run:', forceRun);
  console.log('Active monthly schedules found:', schedules?.length || 0);

  for (const schedule of schedules || []) {
    console.log(`\nChecking schedule ID ${schedule.id}:`);
    console.log(`  Company: ${schedule.company_id}`);
    console.log(`  Scheduled: Day ${schedule.day_of_month} Hour ${schedule.hour} (stored as UK time)`);
    console.log(`  Current:   Day ${currentDay} Hour ${currentHour} (converted UK time)`);
    console.log(`  Match: Day match=${schedule.day_of_month === currentDay}, Hour match=${schedule.hour === currentHour}`);
    
    // Check if should run (either matches schedule OR force run)
    const shouldRun = forceRun || (schedule.day_of_month === currentDay && schedule.hour === currentHour);
    ...
    if (shouldRun) {
      ...
      try {
        ...
        // Generate export for this company - last 30 days
        const monthStart = new Date();
        monthStart.setDate(monthStart.getDate() - 30);
        
        // Get all junior broker user IDs for this company
        ...
        if (juniorUserIds.length === 0) {
          ...
          const messageText = `📊 SpreadChecker Monthly Export - ${now.toLocaleDateString('en-GB')}\n\n` +
            `Period: ${monthStart.toLocaleDateString('en-GB')} - ${now.toLocaleDateString('en-GB')}\n\n` +
        ...
        // Fetch calculations for junior brokers only
        ...
        if (!calculations || calculations.length === 0) {
          console.log(`No calculations found for company ${schedule.company_id} in the last month`);
          
          const messageText = `📊 SpreadChecker Monthly Export - ${now.toLocaleDateString('en-GB')}\n\n` +
            `Period: ${monthStart.toLocaleDateString('en-GB')} - ${now.toLocaleDateString('en-GB')}\n\n` +
            `No calculations recorded this month.\n\n` +
          ...
          let messageText = `📊 SpreadChecker Monthly Export - ${now.toLocaleDateString('en-GB')}\n\n`
          messageText += `Period: ${monthStart.toLocaleDateString('en-GB')} - ${now.toLocaleDateString('en-GB')}\n`
  ...
  return new Response(JSON.stringify({ message: 'Monthly exports completed', forceRun }), { 
    status: 200,
